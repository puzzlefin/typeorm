...
class PostgresDriver {
    ...
    async afterConnect() {
        ...
        if (extensionsMetadata.hasExtensions && !(await this.isReadOnly(this.master))) { // PUZZLE: Check if the connection is read only.
            await Promise.all([this.master].map(pool => {
    ...
    async isReadOnly(connection) {
        try {
            const result = await this.executeQuery(connection, `SHOW transaction_read_only;`);
